...
# -*- coding: utf-8 -*-
"""
CrustaVirus DB Comprehensive Integrity Audit Script
"""
...
for table in core_tables:
    ...
    # Exclude primary keys and certain metadata columns
...
# Check for tables that should have unique constraints but don't
...
if "virus_master" in schema_info:
    cursor.execute("""
        SELECT virus_name, COUNT(*) as cnt FROM virus_master 
        WHERE virus_name IS NOT NULL GROUP BY virus_name HAVING cnt > 1
    """)
    dup_names = cursor.fetchall()
    if dup_names:
        results["constraint_issues"].append({
            "table": "virus_master",
            "issue": "DUPLICATE_VIRUS_NAME",
            "count": len(dup_names),
...
bool_checks = [
    ("viral_isolates", "is_complete_genome"),
    ("viral_isolates", "is_type_species"),
    ("viral_proteins", "is_core_gene"),
]
...
date_columns = [
    ("ref_literatures", "publication_date"),
...
# 7a. viral_isolates.virus_name vs virus_master.virus_name
if "viral_isolates" in schema_info and "virus_master" in schema_info:
    cursor.execute("""
        SELECT DISTINCT vi.virus_name 
        FROM viral_isolates vi 
        LEFT JOIN virus_master vm ON LOWER(vi.virus_name) = LOWER(vm.virus_name)
        WHERE vi.virus_name IS NOT NULL AND vm.virus_name IS NULL
    """)
...
# 7b. sample_collections latitude/longitude range
        ...
        for j in range(len(names)):
            ...
            if len(idx_dict[names[i]]["cols"]) <= len(idx_dict[names[j]]["cols"]):
                if idx_dict[names[i]]["cols"] == idx_dict[names[j]]["cols"][:len(idx_dict[names[i]]["cols"])]:
                    # If shorter is non-unique and longer is unique, shorter might still be useful for some queries
                    # But generally prefix redundancy is still an issue
                    # Only flag if both are non-unique or shorter is also redundant
...
key_columns_to_check = [
    ("viral_isolates", "virus_master_id"),
    ("viral_isolates", "collection_id"),
    ("viral_proteins", "virus_master_id"),
    ("infection_records", "virus_master_id"),
    ("sample_collections", "country"),
    ("sample_collections", "location"),
    ("ref_literatures", "year"),
]
...
# Check if content table exists for FTS
...
for table, info in schema_info.items():
    ...
    if len(pks) > 1:
        results["schema_issues"].append({
            ...
            "pk_columns": pks,
            "severity": "MEDIUM",
            "note": "SQLite does not support AUTOINCREMENT on composite keys; ensure WITHOUT ROWID is intentional"
        })
...
bridge_checks = [
    ...
    ("protein_annotation_bridge", "annotation_id", "interpro_annotations", "annotation_id"),
    ("kegg_protein_pathways", "protein_id", "viral_proteins", "protein_id"),
    ("isolate_reference_links", "isolate_id", "viral_isolates", "isolate_id"),
    ("isolate_reference_links", "reference_id", "ref_literatures", "ref_id"),
]
...
numeric_checks = [
    ("nucleotide_records", "gc_content", 0, 100),
    ("nucleotide_records", "length", 1, 10000000),
...
# ============================================================
# 20. STRICT TABLE CHECK (SQLite 3.37+)
# ============================================================
...
# ============================================================
# 21. ADDITIONAL CHECKS: COORDINATE PRECISION / GEO ISSUES
# ============================================================
...
# ============================================================
# 22. CHECK FOR TABLES WITHOUT ANY INDEX
# ============================================================
