...
def cache_ttl(ttl_seconds: int = _CACHE_TTL):
    """Decorator that caches function results in memory for `ttl_seconds`."""
    def decorator(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            ...
            _CACHE[key] = {"value": result, "expires": now + ttl_seconds}
            return result
        return wrapper
    return decorator
...
DB_VERSION = os.environ.get("AQUAVIR_DB_VERSION", "2.0.0")
...
@app.get("/api/dashboard", tags=["Stats"])
@cache_ttl(300)
def get_dashboard():
    """Aggregate endpoint that returns everything the homepage needs in one call.

    Caches for 5 minutes (300 s) to reduce database load on repeated visits.
    """
    with get_db() as conn:
        c = conn.cursor()
        data = {}
        ...
        c.execute("""
            ...
            WHERE curation_status = 'reviewed'
        """)
        evidence_records = c.fetchone()[0]

        data["counts"] = {
            "viruses": strict_species,
            "isolates": strict_iso,
            "hosts": strict_hosts,
            "evidence": evidence_records,
        }

        # ── 2. Phylum distribution (for homepage bar chart) ─
        c.execute("""
        ...
        """)
        phylum_rows = [dict(r) for r in c.fetchall()]
        data["phylum_distribution"] = phylum_rows
        ...
        c.execute("""
            ...
            LIMIT 10
        """)
        data["top_viruses"] = [dict(r) for r in c.fetchall()]

        # ── 4. Recent additions (by collection_year, fallback 10) ─
        c.execute("""
            SELECT vm.canonical_name AS virus_name, v.accession, v.collection_year
            FROM analysis_strict_target_isolates v
            JOIN virus_master vm ON v.master_id = vm.master_id
            WHERE v.collection_year IS NOT NULL
            ORDER BY v.collection_year DESC
            LIMIT 10
        """)
        data["recent_viruses"] = [dict(r) for r in c.fetchall()]

        # ── 5. Metadata quality (completeness %) ─────────────
        c.execute("""
            SELECT
              ROUND(100.0 * SUM(CASE WHEN vm.canonical_name IS NOT NULL AND vm.canonical_name <> '' THEN 1 ELSE 0 END) / COUNT(*), 1) AS name_pct,
              ROUND(100.0 * SUM(CASE WHEN vm.host_phylum IS NOT NULL THEN 1 ELSE 0 END) / COUNT(*), 1) AS host_pct,
              ROUND(100.0 * SUM(CASE WHEN v.genome_length IS NOT NULL AND v.genome_length > 0 THEN 1 ELSE 0 END) / COUNT(*), 1) AS length_pct,
              ROUND(100.0 * SUM(CASE WHEN v.collection_year IS NOT NULL THEN 1 ELSE 0 END) / COUNT(*), 1) AS year_pct
            FROM analysis_strict_target_isolates v
            JOIN virus_master vm ON v.master_id = vm.master_id
        """)
        quality = dict(c.fetchone())
        data["quality"] = {
            "scores": [
                {"label": "Name", "pct": quality.get("name_pct", 0) or 0},
                {"label": "Host", "pct": quality.get("host_pct", 0) or 0},
                {"label": "Length", "pct": quality.get("length_pct", 0) or 0},
                {"label": "Year", "pct": quality.get("year_pct", 0) or 0},
            ]
        }
...
def serve_about(request: Request):
    """Render about page with citation, version history and data availability info."""
    now = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    versions = [
        {
            "version": "2.0.0",
            "date": "2025-06-12",
            "notes": "Major redesign: progressive disclosure UI, dark mode, semantic colour system, Bento Grid dashboard, single-call /api/dashboard endpoint.",
            "stats": {"tables": 111, "records": "350k+", "isolates": "28k+"},
        },
        {
            "version": "1.5.0",
            "date": "2025-05-20",
            "notes": "Added RDRP BLAST search, protein structure enrichment, KEGG pathway enrichment pages.",
            "stats": {"proteins": "180k+", "structures": "45k+"},
        },
        {
            "version": "1.0.0",
            "date": "2025-04-01",
            "notes": "Initial public release. SQLite-based backend, FastAPI + Jinja2 templates, basic search & browse.",
            "stats": {"isolates": "15k+", "hosts": "1.2k"},
        },
    ]
    stats = [
        {"label": "Virus Species", "value": "4,200+"},
        {"label": "Isolates", "value": "28,000+"},
        {"label": "Host Species", "value": "1,800+"},
        {"label": "Evidence Records", "value": "350,000+"},
        {"label": "Proteins", "value": "180,000+"},
        {"label": "3D Structures", "value": "45,000+"},
        {"label": "Phyla Covered", "value": "9"},
        {"label": "Countries", "value": "42"},
    ]
    tech_stack = [
        {"name": "FastAPI", "role": "Web Framework", "desc": "Python async ASGI framework serving Jinja2 HTML + JSON APIs."},
        {"name": "SQLite + WAL", "role": "Database", "desc": "Single-file relational store with WAL mode for concurrent reads."},
        {"name": "Jinja2", "role": "Templating", "desc": "Server-side HTML generation (MPA, not SPA)."},
        {"name": "HTMX", "role": "Interactivity", "desc": "Lightweight partial-page updates for search results only."},
        {"name": "Tailwind CSS", "role": "Styling", "desc": "Utility-first CSS via CDN for rapid, consistent design."},
        {"name": "ECharts", "role": "Charts", "desc": "Lazy-loaded JS library for geographic maps on detail pages only."},
    ]
    return templates.TemplateResponse(
        request, "about.html",
        {
